<#
	...
	.SYNOPSIS
		This script will audit VMHosts for VCSA and document if they have all
		NFS Datastore.

	.DESCRIPTION
		Use this script to audit all VMHosts in a VCSA
...
$VMHOSTLIST = Get-Cluster $CLUSTER | Get-VMHost | Sort Name
Write-Host (Get-Date -format "MMM-dd-yyyy_HH-mm-ss")
Write-Host "-----------------------------------------------------------------------------------------------------------------------"
...
##Audit VMHosts for NFS
Write-Host "-----------------------------------------------------------------------------------------------------------------------"
Write-Host (Get-Date -format "MMM-dd-yyyy_HH-mm-ss")
Write-Host "Auditing Hosts for NFS"
$VMHOSTLIST = Get-VMHost | Where {$_.ConnectionState -eq "Connected"} | Sort Name
$ARRAY = @()
ForEach($VMHOST in $VMHOSTLIST)
{
	$TEMPARRAY = "" | Select VMHost, NFSDatastore
	$TEMPARRAY.VMHost = $VMHOST.Name
	$DATASTORE = $VMHOST | Get-Datastore | where {$_.Type -eq "NFS41"}
	$TEMPARRAY.NFSDatastore = $DATASTORE.Name
	$ARRAY += $TEMPARRAY
}
Write-Host "Hosts with NFS will show the Datastore name next to them"
Write-Output $ARRAY | Sort VMHost | Format-Table
Write-Host "Completed Auditing Hosts for NFS"
Write-Host (Get-Date -format "MMM-dd-yyyy_HH-mm-ss")
Write-Host "-----------------------------------------------------------------------------------------------------------------------"
