<#
	...
	.SYNOPSIS
		This script removes specified VIBs from a selected cluster of hosts

	.DESCRIPTION
		Use this script to remove the vSphere Replication VIB from a hosts in a
		cluster
...
$LOGFILENAME = "Log_" + $CLUSTER + "_" + $LOGDATE + ".txt"
#Create Log Folder
...
Write-Host "-----------------------------------------------------------------------------------------------------------------------"
Write-Host (Get-Date -format "MMM-dd-yyyy_HH-mm-ss")
Write-Host "Getting VMHost list for Cluster $CLUSTER on VCSA $VCSA"
...
ForEach($VMHOST in $VMHOSTLIST)
{
	...
	#$newsize.height = (get-host).UI.RawUI.MaxWindowSize.Height
	#$newsize.width = 130
	$pswindow.windowtitle = "Removing VIBs on $VMHOST in Cluster $CLUSTER on VCSA $VCSA"
	#$pswindow.windowsize = $newsize
	Write-Host (Get-Date -format "MMM-dd-yyyy_HH-mm-ss")
	Write-Host "-----------------------------------------------------------------------------------------------------------------------"
	...
	$VIBLISTTOREMOVE = 	"vmware-hbr-agent" #VIB for vSphere Replication
	...
	Write-Host "Completed removing VIBS for VMHost $VMHOST in Cluster $CLUSTER on VCSA $VCSA"
