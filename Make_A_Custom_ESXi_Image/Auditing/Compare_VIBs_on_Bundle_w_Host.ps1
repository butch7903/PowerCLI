<#
	...
	.SYNOPSIS
		This script will audit the a single VMHost in a VCSA against an exported CSV
		List of VIBs produced by the Audit_ESXi_Bundle_VIBs.ps1

	.DESCRIPTION
		Use this script to audit all a single host in a VCSA 		
#>
...
Write-Host " "   
$choice = Read-Host "Please select the VMHost from Cluster $CLUSTER as your Reference VMHost"
$REFVMHOST = get-vmhost $VMHOST[$choice]
...
#ESXCLI
$esxcli = Get-EsxCli -VMHost $REFVMHOST -V2
$REFVIBs = $esxcli.software.vib.get.Invoke() | Select Name, ID, Description, Version, CreationDate | Sort Name
...
$VIBDIFF | Where {$_.Name -ne $IGNORELIST} | Sort VMHost,Name | Export-CSV -PATH $EXPORTFILE -NoTypeInformation
