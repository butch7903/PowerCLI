...
$VMHOST = Read-Host "Please provide the VMHost name to audit"
...
##Get VMHost Storage Info
Write-Host "-----------------------------------------------------------------------------------------------------------------------"
Write-Host (Get-Date -format "MMM-dd-yyyy_HH-mm-ss")
Write-Host "Gathering VMHost info from $VMHOST"
Write-Host "Reference VMware KB for more info - https://kb.vmware.com/s/article/2088239"
#ESXCLI
Write-Host "Connecting to $VMHOST via ESXCLI"
$esxcli = Get-EsxCli -VMHost $VMHOST -V2
#Get Storage list
Write-Host "Getting $VMHOST storage list"
$STORAGELIST = $esxcli.storage.core.device.list.invoke()
#List Local Storage
Write-Host "Local Storage on $VMHost includes:"
$STORAGELIST | Where{$_.IsLocal -eq $true}
Write-Host "Use the below list of vendor(s) in your Host Profile > System Image Cache Configuration" -foregroundcolor Green
$STORAGELIST | Where{$_.IsLocal -eq $true} | Select Vendor
Write-Host " "
Write-Host "Completed gathering VMHost info from $VMHOST"
Write-Host (Get-Date -format "MMM-dd-yyyy_HH-mm-ss")
Write-Host "-----------------------------------------------------------------------------------------------------------------------"
